...
- *Share an example of using the command with no arguments.*
  ...
  + the directory before was `/home/lecture1` and after putting in `cd` with no arguments, it went back to the home directory which is `/home` or `~`
...
- *Share an example of using the command with a path to a directory as an argument.*
  ...
  + the directory before was the home directory `/home` and when I used `cd` with the argument `lecture1` it switched the working directory to `/home/lecture1`
...
- *Share an example of using the command with a path to a file as an argument.*
  ...
  + the directory in this case stays the same, before it was `/home` and it is still `/home` or `~` because the git bash gives an error saying `bash: cd: lecture1/Hello.java: Not a directory`
...
- *Share an example of using the command with no arguments.*
  ...
  + the absolute path `/home` does not change after running the command all as `ls` is not functioned to switch directories like `cd`: it's still the home directory
...
- *Share an example of using the command with a path to a directory as an argument.*
  ...
  + the absolute path `/home` does not change at all as `ls` is not functioned to switch directories like `cd`
...
- *Share an example of using the command with a path to a file as an argument.*
  ...
  + the absolute path which is `/home` does not change at all as `ls` is not functioned to switch directories like `cd`
...
- *Share an example of using the command with no arguments.*
  ...
  + working directory was `/home` and it is still `/home`
...
- *Share an example of using the command with a path to a directory as an argument.*
  ...
  + output is an error because cat command only takes files as argument which makes sense since cat is for describing what's in a file
- *Share an example of using the command with a path to a file as an argument.*
